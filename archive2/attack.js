        ...
        if (prepAttack) {
            ...
            if (prepAttack.cycleThreads < freeThreads || allowRamOverflow) {
                ...
                if (allowRamOverflow && prepAttack.parts.g.threads) {
                    ...
                    prepAttack.parts.gw.threads = Math.ceil((prepAttack.parts.g.threads * AttackSettings.scripts.g.change) / AttackSettings.scripts.w.change)
...
export async function runCracks(ns, servers) {
    ...
    for (const server of rootableServers) {
        ...
        // copy hack scripts
        await ns.scp(ServerSettings.hackScripts, server.hostname)
