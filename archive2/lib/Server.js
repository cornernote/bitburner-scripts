 ...
 * @return {Server[]}
 */
export async function getServers(runner) {
    const servers = []
    for (const hostname of scanAll(runner)) {
        const server = runner.nsProxy['getServer'](hostname)
        if (server.hostname === 'home') {
            server.ramUsed += ServerSettings.homeReservedRam
        }
        servers.push(server)
    }
    return servers
 ...
 * @return {string[]}
 */
export async function scanAll(runner) {
    const servers = []
    const spider = ['home']
    while (spider.length > 0) {
        const hostname = spider.pop()
        for (const scanned of runner.nsProxy['scan'](hostname)) {
